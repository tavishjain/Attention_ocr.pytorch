...
class  AttentionCell ( nn . Module ):
    ...
    def forward(self, prev_hidden, feats, cur_embeddings):
        ...
        context  = ( feats  *  alpha . transpose ( 0 , 1 ). contiguous (). view ( nT , nB , 1 ). expand ( nT , nB , nC )). sum ( 0 ). squeeze ( 0 ) # nB * nC //It feels like it should not be sum, output 4×256
...
class Attentiondecoder(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        embedded  =  self . embedding ( input )          # The previous output is word embedding
        embedded = self.dropout(embedded)

        attn_weights = F.softmax(
            self . attn ( torch . cat (( embedded , hidden [ 0 ]), 1 )), dim = 1 )         # The last output and hidden state are used to calculate the weights, mainly using a linear layer from 512 dimensions to 71 dimensions, so Can only handle fixed-width sequences
...
def target_txt_decode(batch_size, text_length, text):
    ...
    targets  =  torch . ones ( nB , num_steps  +  2 ) *  2                  # Fill a shorter string with the $ sign, add GO at the beginning and EOS_TOKEN at the end
    targets = targets.long().cuda()        # 用
    start_id = 0
    for  i  in  range ( nB ):
        targets [ i ][ 0 ] =  GO     # Add a start tag at the beginning
        targets[i][1:text_length.data[i] + 1] = text.data[start_id:start_id+text_length.data[i]]       # 是否要加1
        targets [ i ][ text_length . data [ i ] +  1 ] =  EOS_TOKEN          # Add end tag
        start_id  =  start_id + text_length . data [ i ]                  # Split the target label of each target, as: batch×numel of the longest character
...
class CNN(nn.Module):
    '''
        CNN+BiLstm for feature extraction
    '''
    ...
    def forward(self, input):
        ...
        encoder_outputs  =  self . rnn ( conv )           # seq * batch * n_classes// 25 × batchsize × 256 (the number of hidden nodes)
...
class AttentiondecoderV2(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        Output  =  F. . log_softmax ( Self . OUT ( Output [ 0 ]), Dim = . 1 )           # final output probability
        return output, hidden, attn_weights
